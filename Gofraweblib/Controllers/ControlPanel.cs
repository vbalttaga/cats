...
namespace Gofraweblib.Controllers
{
    ...
    public class ControlPanel : Weblib.Controllers.ControlPanel
    {
        ...
        public ActionResult EditItem(string BO, string Namespace, long Id)
        {
            ...
            var item = Activator.CreateInstance(Type.GetType(Namespace + "." + BO + ", " + Namespace.Split('.')[0], true));
            ViewData["BOType"] = item.GetType();
            ViewData["Back_Link"] = LIB.Tools.Utils.URLHelper.GetUrl("ControlPanel/Edit/" + item.GetType().Name + "/" + HttpUtility.UrlEncode(item.GetType().Namespace));
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["BOLink"]))
            {
                ...
                ViewData["Back_Link"] = LIB.Tools.Utils.URLHelper.GetUrl("ControlPanel/Edit/" + item.GetType().Name + "/" + HttpUtility.UrlEncode(item.GetType().Namespace) + "/" + BOLink + "/" + HttpUtility.UrlEncode(NamespaceLink) + "/" + IdLink.ToString());
            }
            else
            {
                ViewData["LinkItem"] = null;
            ...
            if (item != null)
            {
                ...
                {
                    if (!Authentication.GetCurrentUser().HasAtLeastOnePermission(((GofraLib.BusinessObjects.User)item).Role.RoleAccessPermission))
                    {
        ...
        public ActionResult Profile(string Id)
        {
            ...
            if (!LIB.Tools.Security.Authentication.GetCurrentUser().HasPermissions((long)BasePermissionenum.CPAccess) && UserId != LIB.Tools.Security.Authentication.GetCurrentUser().Id)
            {
                return View("NoAccess");
            }
